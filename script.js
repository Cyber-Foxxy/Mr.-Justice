...
class Player {
  ...
  draw() {
    ...
    glow.addColorStop(0, "rgba(238, 246, 246, 0.55)");
    glow.addColorStop(0.35, "rgba(235, 232, 13, 0.18)");
    glow.addColorStop(1, "rgba(237, 231, 223, 0.14)");
    ctx.fillStyle = glow;
    ctx.beginPath();
...
class Rune {
  ...
  draw() {
    ...
    glow.addColorStop(1, "rgba(239, 238, 229, 0)");
    ctx.fillStyle = glow;
    ctx.beginPath();
...
function drawBackground() {
  ...
  sky.addColorStop(0, "#0e3ed0a4");
  sky.addColorStop(0.55, "#eceaf5");
  sky.addColorStop(1, "#8282f3");
  ctx.fillStyle = sky;
...
function drawMoonPortal() {
  ...
  if (needed > 0 && collected === needed) {
    ctx.fillStyle = "rgba(255, 251, 124, 0.9)";
    ctx.beginPath();
    ctx.arc(0, 0, 72 + Math.sin(animationTime * 0.05) * 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#f8972f6b";
...
function drawDarknessOverlay() {
  ctx.save();
  ctx.fillStyle = "rgba(232, 220, 220, 0)";
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  ctx.globalCompositeOperation = "destination-out";
  const glow = ctx.createRadialGradient(player.x, player.y - 48, 30, player.x, player.y - 48, 185);
  glow.addColorStop(0, "rgba(244, 242, 242, 0.22)");
  glow.addColorStop(0.6, "rgba(242, 244, 137, 0.17)");
  glow.addColorStop(1, "rgba(251, 110, 110, 0)");
  ctx.fillStyle = glow;
  ctx.beginPath();
